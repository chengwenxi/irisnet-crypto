...
function serializePath (path) {
    ...
    const buf = Buffer.alloc(4 * path.length)
...
module.exports = class IrisApp {
    ...
    async deviceInfo () {
        return this.transport.send(0xE0, 0x01, 0, 0, Buffer.from([]), [0x9000, 0x6E00]).then(
            response => {
                const errorCodeData = response.slice(-2)
                const returnCode = errorCodeData[0] * 256 + errorCodeData[1]
    ...
    async publicKey (path) {
        ...
        return this.transport.send(CLA, INS.PUBLIC_KEY_SECP256K1, 0, 0, serializedPath).then(
            response => {
                const errorCodeData = response.slice(-2)
                const returnCode = errorCodeData[0] * 256 + errorCodeData[1]
    ...
    async getAddressAndPubKey (path, hrp) {
        ...
        return this.transport.send(CLA, INS.GET_ADDR_SECP256K1, 0, 0, data, [0x9000]).then(
            response => {
                const errorCodeData = response.slice(-2)
                const returnCode = errorCodeData[0] * 256 + errorCodeData[1]
